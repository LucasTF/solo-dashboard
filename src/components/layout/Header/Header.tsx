...
const Header = () => {
  ...
  return (
    <header className="h-14 max-h-14 border-b-2 border-slate-400 bg-slate-200 dark:bg-zinc-800 dark:border-zinc-900 sticky top-0 left-0 z-10">
      <div className="lg:hidden h-full w-full flex justify-between items-center px-5">
        <button
          className="bg-slate-200 dark:bg-zinc-900 dark:bg-none rounded-md p-2"
          type="button"
      ...
      <nav className={drawer({ open: isDrawerOpen })}>
        ...
        <div className="lg:hidden bg-slate-300 dark:bg-gray-800 border border-b-slate-400 dark:border-zinc-900 p-4 flex flex-col gap-4 justify-center items-center">
          ...
          {isPending ? (
            <p className="lg:hidden font-bold">Saindo...</p>
          ) : (
            <button
              type="button"
        ...
        {isPending ? (
          ...
          <ul className="flex gap-4 items-center max-lg:hidden">
            ...
            <li>
              <button
                type="button"
                className="flex gap-1 p-2 hover:text-red-600 ease-in-out duration-300"
                onClick={() => logoutHandler()}
              >
                <ArrowLeftStartOnRectangleIcon className="size-6" />
